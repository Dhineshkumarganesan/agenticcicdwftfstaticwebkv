...
- Figure 3: VS Code file explorer showing scaffolded Terraform files for all environments
...
- Figure 4: Example of scaffolded main.tf generated by the agent
...
- Figure 5: Example of scaffolded outputs.tf generated by the agent
...
- Figure 7: Terraform plan output for dev environment showing resource creation `+azurerm_resource_group.main` and `+azurerm_storage_account.web`
...
- Figure 9: A second prompt triggered a real infrastructure change that kicked off CD ✓
...
- Figure 10: GitHub Actions CD run — progressive deploys to dev, test, and prod — all 3 deploy jobs green (dev → test → prod in sequence)

<img width="1875" height="792" alt="image" src="https://github.com/user-attachments/assets/fd88307c-a4fb-4735-8e8e-535b2397a184" />

- Figure 11: Azure Portal showing created Resource Groups for each environment `rg-agfactory-dev`, `rg-agfactory-test`, `rg-agfactory-prod`
   
<img width="1910" height="632" alt="image" src="https://github.com/user-attachments/assets/2f95a935-5133-422e-9437-33a7c7a566fd" />

- Figure 12: Azure Portal showing Storage Account `stagfactorydev` with Static website enabled
...
- Figure 13: GitHub Actions Destroy workflow dispatch with DESTROY confirmation (environment=all, confirm=DESTROY)
...
- Figure 15: Azure Portal after Resource Groups deletion (clean state).We are left with only terraform state resource group which will be cleaned up at the end
