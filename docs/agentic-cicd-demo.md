...
---
Traditional CI/CD pipelines are often rigid and require developers to explicitly define every build, test, and deployment step, making it hard to scale across multiple projects or rapidly changing requirements. Intent-driven CI/CD solves this by understanding what you want to deploy and automatically scaffolding the necessary pipeline steps, reducing manual configuration, minimizing errors, and speeding up delivery. This approach is especially valuable in complex, multi-repo, or agentic AI environments where deployments must be dynamic, repeatable, and aligned with business intent.


## The Two-Repo Architecture

Two repositories power this demo — here’s how they fit together.:
...
- Figure 1: Azure Portal — App Registration "agenticcicdwftfstaticwebkv-oidc" → Federated credentials tab showing all 5 credentials
...
```
 ...
 - Figure 2: Declaring deployment intent in cicd/contract.yml (VS Code)
...
- Figure 3: Copilot Chat with @terraform-module-expert prompt and agent response

<img width="1116" height="776" alt="image" src="https://github.com/user-attachments/assets/e65a4b2c-9c31-4fc2-afd9-fdcc1e2689aa" />

Scaffold Summary

- Figure 4: VS Code file explorer showing scaffolded Terraform files for all environments

<img width="1127" height="742" alt="image" src="https://github.com/user-attachments/assets/c557e768-4200-452d-9f17-774249970755" />

how infra/envs/dev/main.tf` file scaffolded

- Figure 5: Example of scaffolded main.tf generated by the agent
...
- Figure 6: Example of scaffolded outputs.tf generated by the agent
...
- Figure 8: Terraform plan output for dev environment showing resource creation `+azurerm_resource_group.main` and `+azurerm_storage_account.web`
...
- Figure 10: A second prompt triggered a real infrastructure change that kicked off CD ✓
...
- Figure 11: GitHub Actions CD run — progressive deploys to dev, test, and prod — all 3 deploy jobs green (dev → test → prod in sequence)

<img width="1875" height="792" alt="image" src="https://github.com/user-attachments/assets/fd88307c-a4fb-4735-8e8e-535b2397a184" />

- Figure 12: Azure Portal showing created Resource Groups for each environment `rg-agfactory-dev`, `rg-agfactory-test`, `rg-agfactory-prod`
   
<img width="1910" height="632" alt="image" src="https://github.com/user-attachments/assets/2f95a935-5133-422e-9437-33a7c7a566fd" />

- Figure 13: Azure Portal showing Storage Account `stagfactorydev` with Static website enabled
...
- Figure 14: GitHub Actions Destroy workflow dispatch with DESTROY confirmation (environment=all, confirm=DESTROY)

<img width="1883" height="861" alt="image" src="https://github.com/user-attachments/assets/a465ce1d-f12a-476d-9643-a9dfa8ff4c68" />

- Figure 15: Destroy workflow run — all jobs passing
...
5. **The agent is not magic** — it reads your contract and writes standard Terraform. We review it, we own it, and CI ensures it’s correct before deployment to the cloud.
